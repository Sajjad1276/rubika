...
async def audit(
    ...
    *,
    ...
    row = AuditLog(actor_id=actor_id, action=action, entity_type=entity_type, entity_id=entity_id,
...
class RegistrationService:
    ...
    async def start(self, *, rubika_guid: str, applicant: User, source: RegistrationSource,
        ...
        request = RegistrationRequest(channel_id=channel.id, applicant_id=applicant.id, source=source,
                                      recruited_by_admin_id=recruited_by_admin_id,
                                      assigned_admin_id=recruited_by_admin_id, list_id=list_id,
                                      status=RegistrationStatus.DRAFT)
        self.db.add(request); await self.db.flush()
    ...
    async def submit_for_verification(self, request: RegistrationRequest) -> RegistrationRequest:
        ...
        request.status = RegistrationStatus.PENDING_VERIFICATION; await self.db.flush(); return request
    ...
    async def verify(self, request: RegistrationRequest, *, approved: bool,
        ...
        if approved:
            if not request.list_id: raise ValueError("Approved registration requires a list")
            ...
            channel.list_id = request.list_id; channel.status = ChannelStatus.ACTIVE
            if member_count is not None: channel.member_count = member_count
            channel.list_code = await ListService(self.db).allocate_channel_code(request.list_id)
            request.status = RegistrationStatus.ACTIVE; request.completed_at = datetime.now(timezone.utc)
        else:
            channel.status = ChannelStatus.PENDING; request.status = RegistrationStatus.REJECTED
        await self.db.flush()
...
class VerificationService:
    ...
    async def verify_from_rubika(self, *, channel_id: str, gateway: RubikaGateway,
                                 operational_account_user_id: str, verifier_id: str | None = None) -> Channel:
        ...
        channel.member_count = snapshot.member_count; channel.username = snapshot.username; channel.title = snapshot.title
        channel.can_send = access.can_send; channel.can_edit = access.can_edit; channel.can_delete = access.can_delete
        channel.access_verified = access.is_admin and access.can_send and access.can_edit and access.can_delete
        channel.verification_notes = ("verified from Rubika" if channel.access_verified else
                                      "Rubika account is not an administrator with all required permissions")
        await self.db.flush()
        await audit(self.db, actor_id=verifier_id, action="channel_permissions_verified_from_rubika",
                    entity_type="channel", entity_id=channel_id,
                    metadata={"account_user_id": operational_account_user_id, "member_count": channel.member_count,
                              "is_admin": access.is_admin, "can_send": access.can_send,
                              "can_edit": access.can_edit, "can_delete": access.can_delete})
        return channel

    async def mark_permissions(self, channel_id: str, *, verified_by: str, can_send: bool,
                               can_edit: bool, can_delete: bool, notes: str | None = None) -> Channel:
        channel = await self.db.get(Channel, channel_id)
        if channel is None: raise ValueError("Channel not found")
        channel.can_send = can_send; channel.can_edit = can_edit; channel.can_delete = can_delete
        channel.access_verified = can_send and can_edit and can_delete; channel.verification_notes = notes
        await self.db.flush()
...
class ListService:
    ...
    async def allocate_channel_code(self, list_id: str) -> str:
        ...
        if network_list is None: raise ValueError("List not found")
        number = network_list.next_channel_number; network_list.next_channel_number = number + 1
        await self.db.flush(); return f"#{number:03d}"

    async def active_channel_count(self, list_id: str) -> int:
        count = await self.db.scalar(select(func.count(Channel.id)).where(Channel.list_id == list_id,
...
class TaskService:
    ...
    async def create(self, *, assignee_id: str, task_type: str, payload: dict | None = None,
...
class ViolationService:
    ...
    async def record(self, *, channel_id: str, violation_type: str, severity: int = 1,
        ...
        violation = Violation(channel_id=channel_id, admin_id=admin_id, violation_type=violation_type,
                              severity=max(1, severity), note=note)
        self.db.add(violation); await self.db.flush()
        await audit(self.db, actor_id=admin_id, action="violation_recorded", entity_type="channel",
                    entity_id=channel_id, metadata={"type": violation_type, "severity": severity})
        return violation
