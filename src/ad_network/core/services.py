    ...
    async def start(
        ...
        *,
        ...
        existing = await self.db.scalar(select(RegistrationRequest).where(
            ...
            RegistrationRequest.applicant_id == applicant.id,
            RegistrationRequest.list_id == list_id,
    ...
    async def verify_from_rubika(
        ...
        *,
        ...
        channel.verification_notes = (
            "verified from Rubika"
            if channel.access_verified
    ...
    async def mark_permissions(
        ...
        *,
        ...
        await audit(
            ...
            action="channel_permissions_verified",
            entity_type="channel",
            entity_id=channel_id,
...
class ListService:
    ...
    async def allocate_channel_code(self, list_id: str) -> str:
        network_list = await self.db.scalar(
            select(ListNetwork).where(ListNetwork.id == list_id).with_for_update()
        )
