...
class OrderFlowService:
    ...
    async def schedule_paid_order(
        ...
        await RotationPlanner(self.db).plan(campaign, order.list_id, start_at=start_at, interval_seconds=interval_seconds)
        ...
        await CommerceService(self.db).settle(
            order, list_beneficiary_id=list_beneficiary_id, admin_beneficiary_id=admin_beneficiary_id,
        )
