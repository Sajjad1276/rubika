...
class FastRubikaGateway:
    """Rubika transport adapter. Business logic must not depend on fast_rub."""
    ...
    async def get_channel(self, guid: str) -> ChannelSnapshot:
        ...
        return ChannelSnapshot(guid=guid, title=channel.get("title"), username=channel.get("username"), member_count=channel.get("members_count"))
    async def forward(self, source_guid: str, target_guid: str, message_id: str) -> Any:
