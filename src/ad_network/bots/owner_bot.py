...
async def build_owner_bot(settings: Settings) -> Client:
    ...
    async def handle_action(message, action: str):
        ...
        async with SessionFactory() as db:
            ...
            if action == "home":
                await db.commit(); await reply(message, "👑 مدیریت شبکه", keypad=owner_keyboard()); return
            if action == "overview":
            ...
            if action == "lists":
                ...
                await db.commit(); await reply(message, "🗂 لیست‌ها\n\n" + ("\n".join(lines) or "لیستی ثبت نشده است."), inline_keypad=inline_keyboard((("list:add", "➕ ساخت لیست"),), (("home", "↩️ بازگشت"),))); return
            if action == "list:add":
                _STATES[user_id] = "list"; await db.commit(); await reply(message, "➕ ساخت لیست\nفرمت: کد | نام لیست | حداقل کانال\nمثال: 001 | سحابی | 20"); return
            if action == "pricing":
                ...
                await db.commit(); await reply(message, "💰 تعرفه‌های فعال\n\n" + ("\n".join(lines) or "هنوز تعرفه‌ای ثبت نشده است."), inline_keypad=inline_keyboard((("price:add", "➕ ثبت تعرفه"),), (("home", "↩️ بازگشت"),))); return
            if action == "price:add":
                _STATES[user_id] = "price"; await db.commit(); await reply(message, "➕ ثبت تعرفه\nفرمت: کد لیست | عنوان | حداقل کانال | قیمت هر کانال | ساعت ماندگاری\nمثال: 001 | پایه | 20 | 50000 | 6"); return
            responses = {
            ...
            if action in responses:
                await reply(message, responses[action], keypad=owner_keyboard())
    ...
    async def handle_menu_text(message, text: str) -> bool:
        mapping = {
            "📊 نمای کلی": "overview", "🗂 لیست‌ها": "lists",
            "👥 ادمین‌ها و ناظرها": "people", "📢 سفارش‌ها": "orders",
            "💰 تعرفه و درآمد": "pricing", "📚 قوانین": "rules",
            "🧾 Audit": "audit", "⚙️ ماژول‌ها": "modules",
        }
    ...
    async def handle(message):
        ...
        if text in {"/start", "منو", "menu"}:
            await reply(message, "👑 مدیریت شبکه", keypad=owner_keyboard()); return
        if await handle_menu_text(message, text):
            return
        if user_id in _STATES:
            async with SessionFactory() as db:
                ...
                user = await roles.get_or_create_user(rubika_user_id=user_id)
                if not roles.has(user, UserRole.OWNER, UserRole.SUPERVISOR):
                    await db.commit(); _STATES.pop(user_id, None); await reply(message, "⛔ دسترسی ندارید."); return
                state = _STATES[user_id]
                parts = [x.strip() for x in text.split("|")]
                try:
                    if state == "list":
                        if len(parts) != 3 or not parts[0] or not parts[1]: raise ValueError("فرمت: کد | نام لیست | حداقل کانال")
                        code, name = parts[0].lstrip("#"), parts[1]; min_channels = int(parts[2])
                        ...
                        if exists: raise ValueError("این کد لیست قبلاً استفاده شده است")
                    ...
                    if state == "price":
                        ...
                        code, title = parts[0].lstrip("#"), parts[1]; min_channels, price_per_channel, retention_hours = map(int, parts[2:5])
                        ...
                        await db.commit(); _STATES.pop(user_id, None); await reply(message, "✅ تعرفه ثبت شد.", keypad=owner_keyboard()); return
                except (ValueError, TypeError):
                    await db.rollback(); await reply(message, "❌ فرمت یا مقدار نامعتبر است. دوباره ارسال کن."); return
        async with SessionFactory() as db:
            roles = RoleService(db, settings)
            user = await roles.get_or_create_user(rubika_user_id=user_id)
            ...
            await db.commit()
        if not allowed:
            await reply(message, "⛔ دسترسی این ربات فقط برای مالک و ناظر شبکه است."); return
        if text == "3":
