...
async def build_admin_bot(
        ...
        async with SessionFactory() as db:
            ...
            if action == "accounts" or action.startswith("account:"):
                ...
                if account_runtime is None:
                    await reply(event, "⛔ Runtime اکانت‌ها در دسترس نیست.")
                else:
            ...
            if action.startswith("approve:"):
                ...
                try:
                    ...
                    await db.commit(); await reply(event, "✅ کانال فعال شد.", keypad=admin_keyboard())
                except ValueError as exc:
    ...
    mapping = {
        "📥 درخواست‌ها": "requests", "💳 پرداخت‌ها": "payments", "👤 اکانت‌های لیست": "accounts", "📺 کانال‌ها": "channels",
        "📋 وظایف": "tasks", "🎯 جذب کانال": "recruit", "📣 عملیات تبلیغ": "campaigns", "⚠️ تخلفات": "violations",
